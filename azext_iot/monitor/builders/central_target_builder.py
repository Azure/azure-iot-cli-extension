...
def build_central_event_hub_targets(cmd, app_id, central_api_uri) -> List[Target]:
    event_loop = asyncio.get_event_loop()
    return event_loop.run_until_complete(
        _build_central_event_hub_targets_async(cmd, app_id, central_api_uri)
    )


async def _build_central_event_hub_targets_async(cmd, app_id, central_api_uri):
