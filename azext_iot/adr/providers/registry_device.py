...
def _profile_metadata(profile: Optional[dict]) -> Optional[dict]:
    """Never expose symmetric credentials through the metadata commands.

    The modeless SDK's polymorphic GET shape includes symmetricKey. Do not rely
    on a particular backend build to omit it. Copy, rather than mutate, the
    response, and keep list-keys as the only explicit secret-returning action.
    """
    if profile is None:
        return None
    result = dict(profile)
    if isinstance(result.get("properties"), dict):
        ...
        result["properties"].pop("symmetricKey", None)
    return result
...
class RegistryDeviceProvider(ADRProvider):
    ...
    def auth_list(
        ...
        return [
            ...
            for profile in self.client.registry_device_authentication_profiles.list_by_device(
                resource_group_name=resource_group_name,
                namespace_name=namespace_name,
                registry_device_name=registry_device_name,
    ...
    def auth_show(
        ...
        return _profile_metadata(self.client.registry_device_authentication_profiles.get(
            resource_group_name=resource_group_name,
            namespace_name=namespace_name,
            registry_device_name=registry_device_name,
