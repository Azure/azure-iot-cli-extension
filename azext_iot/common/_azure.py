...
def get_iot_pnp_connection_string(
    cmd, endpoint, repo_id, user_role="Admin", login=None
):
    """
    ...
    from azure.cli.command_modules.iot.digitaltwinrepositoryprovisioningservice import (
    ...
    def _find_key_from_list(keys, user_role):
        if keys:
            return next(
                (key for key in keys if key.user_role.lower() == user_role.lower()),
                None,
            )
        return None
    ...
    if repo_id:
        ...
        if policy is None:
            raise CLIError(
                'No auth key found for repository "{}" with user_role "{}".'.format(
                    repo_id, user_role
                )
            )
