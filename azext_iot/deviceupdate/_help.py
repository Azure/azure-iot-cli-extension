...
def load_deviceupdate_help():
    ...
    helps["iot device-update update"] = """
        type: group
    ...
    helps["iot device-update update list"] = """
        ...
        - name: List all update providers.
          text: >
            az iot device-update update list -n {account_name} -i {instance_name} --by-provider
    ...
    helps["iot device-update update show"] = """
        ...
        - name: Show a specific update with respect to update provider, name and version.
          text: >
            az iot device-update update show -n {account_name} -i {instance_name} --update-provider {provider_name}
            --update-name {update_name} --update-version {update_version}
    """

    helps["iot device-update update delete"] = """
        type: command
        ...
        - name: Delete a target update with respect to update provider, name and version.
          text: >
            az iot device-update update delete -n {account_name} -i {instance_name} --update-provider {provider_name}
            --update-name {update_name} --update-version {update_version}
    """

    helps["iot device-update update file"] = """
        type: group
        short-summary: Update file operations.
    """

    helps["iot device-update update file list"] = """
        type: command
        ...
        - name: List update files with respect to update provider, name and version.
          text: >
            az iot device-update update file list -n {account_name} -i {instance_name} --update-provider {provider_name}
            --update-name {update_name} --update-version {update_version}
    """

    helps["iot device-update update file show"] = """
        type: command
        ...
        - name: Show a specific update file with respect to update provider, name and version.
          text: >
            az iot device-update update file show -n {account_name} -i {instance_name} --update-provider {provider_name}
            --update-name {update_name} --update-version {update_version} --update-file-id {update_file_id}
    """

    helps["iot device-update update import"] = """
        type: command
        ...
        - name: Import an update with two related files and no reference steps, explicitly providing
          ...
          text: >
            az iot device-update update import -n {account_name} -i {instance_name} --hashes sha256={hash_value} --size {size_in_bytes}
        ...
        - name: Import an update with two related files and no reference steps, letting the CLI calculate the update manifest
          ...
          text: >
            az iot device-update update import -n {account_name} -i {instance_name} --url {manifest_location}
        ...
        - name: Import a parent update with two child update reference steps, where all three update manifests have one related file.
          ...
          text: >
            az iot device-update update import -n {account_name} -i {instance_name} --url {parent_manifest_location}
            --file filename={parent_file_name} url={parent_file_url} --defer


            az iot device-update update import -n {account_name} -i {instance_name} --url {child1_manifest_location}
            --file filename={child1_file_name} url={child1_file_url} --defer


            az iot device-update update import -n {account_name} -i {instance_name} --url {child2_manifest_location}
            --file filename={child2_file_name} url={child2_file_url}
    """

    helps["iot device-update device"] = """
        type: group
        short-summary: Device Update device management.
    """

    helps["iot device-update device class"] = """
        type: group
    ...
    helps["iot device-update device class list"] = """
        type: command
    ...
    helps["iot device-update device class show"] = """
        type: command
    ...
    helps["iot device-update device class update"] = """
        type: command
    ...
    helps["iot device-update device class delete"] = """
        type: command
    ...
    helps["iot device-update device group"] = """
        type: group
    ...
    helps["iot device-update device group list"] = """
        type: command
        ...
        - name: List device groups.
          text: >
            az iot device-update device group list -n {account_name} -i {instance_name}

        - name: List device groups in a desired order, in this case by deviceCount.
          text: >
            az iot device-update device group list -n {account_name} -i {instance_name} --order-by deviceCount
    """

    helps["iot device-update device group show"] = """
        type: command
        ...
        - name: Show the best updates available for a device group. This flag modifies the command to returns a list.
          text: >
            az iot device-update device group show -n {account_name} -i {instance_name} --group-id {device_group_id} --best-updates

        - name: Show update compliance for a device group.
          text: >
            az iot device-update device group show -n {account_name} -i {instance_name} --group-id {device_group_id} --update-compliance
    """

    helps["iot device-update device group delete"] = """
        type: command
        ...
        - name: Delete a device group skipping the confirmation prompt.
          text: >
            az iot device-update device group delete -n {account_name} -i {instance_name} --group-id {device_group_id} -y
    """

    helps["iot device-update device deployment"] = """
        type: group
    ...
    helps["iot device-update device deployment create"] = """
        type: command
        ...
        - name: Create a device group deployment scheduled to start immediately (with respect to UTC time).
          text: >
            az iot device-update device deployment create -n {account_name} -i {instance_name} --group-id {device_group_id} --deployment-id {deployment_id}
    ...
    helps["iot device-update device deployment list"] = """
        type: command
        ...
        - name: List deployments for a device group.
          text: >
            az iot device-update device deployment list -n {account_name} -i {instance_name} --group-id {device_group_id}

        - name: List deployments for a device group ordering results by startDateTime descending.
          text: >
            az iot device-update device deployment list -n {account_name} -i {instance_name} --group-id {device_group_id} --order-by "startDateTime desc"

        - name: List deployments for a device class subgroup.
          text: >
            az iot device-update device deployment list -n {account_name} -i {instance_name} --group-id {device_group_id} --class-id {device_class_id}
    """

    helps["iot device-update device deployment list-devices"] = """
        type: command
        ...
        - name: List devices in a device class subgroup deployment.
          text: >
            az iot device-update device deployment list-devices -n {account_name} -i {instance_name} --group-id {device_group_id}
    ...
    helps["iot device-update device deployment show"] = """
        type: command
        ...
        - name: Show a deployment for a device group.
          text: >
            az iot device-update device deployment show -n {account_name} -i {instance_name} --group-id {device_group_id}
    ...
    helps["iot device-update device deployment cancel"] = """
        type: command
        ...
        - name: Cancel the target device class subgroup deployment.
          text: >
            az iot device-update device deployment cancel -n {account_name} -i {instance_name} --deployment-id {deployment_id}
            --group-id {device_group_id} --class-id {device_class_id}
    """

    helps["iot device-update device deployment retry"] = """
        type: command
        ...
        - name: Retry the target device class subgroup deployment.
          text: >
            az iot device-update device deployment retry -n {account_name} -i {instance_name} --deployment-id {deployment_id}
            --group-id {device_group_id} --class-id {device_class_id}
    """

    helps["iot device-update device deployment delete"] = """
        type: command
        ...
        - name: Delete the target device group deployment.
          text: >
            az iot device-update device deployment delete -n {account_name} -i {instance_name} --deployment-id {deployment_id}
    ...
    helps["iot device-update device compliance"] = """
        type: group
        short-summary: Instance-view device compliance utilities.
    """

    helps["iot device-update device compliance show"] = """
        type: command
        ...
        - name: Show device update compliance for an instance.
          text: >
            az iot device-update device compliance show -n {account_name} -i {instance_name}
    """

    helps["iot device-update device health"] = """
        type: group
        short-summary: Device health-check utilities.
    """

    helps["iot device-update device health list"] = """
        type: command
        ...
        - name: List healthy devices in an instance.
          text: >
            az iot device-update device health list -n {account_name} -i {instance_name} --filter "state eq 'Healthy'"

        - name: List unhealthy devices in an instance.
          text: >
            az iot device-update device health list -n {account_name} -i {instance_name} --filter "state eq 'Unhealthy'"

        - name: Show the health state for a target device.
          text: >
            az iot device-update device health list -n {account_name} -i {instance_name} --filter "deviceId eq 'd0'"
    """

    helps["iot device-update device log"] = """
        type: group
        short-summary: Instance log collection management.
    """

    helps["iot device-update device log collect"] = """
        type: command
        ...
        - name: Configure diagnostics log collection for two devices d0 and d1.
          text: >
            az iot device-update device log collect -n {account_name} -i {instance_name} --log-collection-id {log_collection_id} --description "North-wing device diagnostics"
            --agent-id deviceId=d0 --agent-id deviceId=d1

        - name: Configure diagnostics log collection for a module m0 deployed on device d0.
          text: >
            az iot device-update device log collect -n {account_name} -i {instance_name} --log-collection-id {log_collection_id} --description "ML module diagnostics"
            --agent-id deviceId=d0 moduleId=m0
    """

    helps["iot device-update device log list"] = """
        type: command
        ...
        - name: List diagnostic log collection operations.
          text: >
            az iot device-update device log list -n {account_name} -i {instance_name}
    """

    helps["iot device-update device log show"] = """
        type: command
        ...
        - name: Show a diagnostic log collection operation.
          text: >
            az iot device-update device log show -n {account_name} -i {instance_name} --log-collection-id {log_collection_id}
    """

    helps["iot device-update device module"] = """
        type: group
        short-summary: Device module management.
    """

    helps["iot device-update device module show"] = """
        type: command
        ...
        - name: Show a device module identity.
          text: >
            az iot device-update device module show -n {account_name} -i {instance_name} -d {device_id} -m {module_id}
    """

    helps["iot device-update device import"] = """
        type: command
        ...
        - name: Import devices and modules into a target instance.
          text: >
            az iot device-update device import -n {account_name} -i {instance_name}

        - name: Import only devices.
          text: >
            az iot device-update device import -n {account_name} -i {instance_name} --import-type Devices

        - name: Import only modules.
          text: >
            az iot device-update device import -n {account_name} -i {instance_name} --import-type Modules
    """

    helps["iot device-update device list"] = """
        type: command
        ...
        - name: List device identities within a target instance.
          text: >
            az iot device-update device list -n {account_name} -i {instance_name}

        - name: List device identities within a target instance filtering by a desired group Id.
          text: >
            az iot device-update device list -n {account_name} -i {instance_name} --filter "groupId eq 'myDeviceGroup'"
    """

    helps["iot device-update device show"] = """
        type: command
