...
from azext_iot.operations.pnp import (
    iot_pnp_interface_show,
...
def iot_digitaltwin_interface_list(
    cmd, device_id, hub_name=None, resource_group_name=None, login=None
):
    device_default_interface = _iot_digitaltwin_interface_show(
        cmd, device_id, INTERFACE_KEY_NAME, hub_name, resource_group_name, login
    )
...
def iot_digitaltwin_properties_list(
    cmd,
    device_id,
    source_model,
    interface=None,
    ...
    for entity in target_interfaces:
        ...
        if source_model == ModelSourceType.device.value.lower():
            ...
            if source_model == ModelSourceType.private.value.lower():
                _validate_repository(repo_id, repo_login)
            found_properties = _pnp_interface_elements(
                cmd,
                entity["urn_id"],
                INTERFACE_PROPERTY,
                repo_endpoint,
...
def iot_digitaltwin_monitor_events(
    cmd,
    hub_name=None,
    device_id=None,
...
def _get_device_default_interface_dict(device_default_interface):
    ...
    result = []
    for k, v in interface["properties"]["modelInformation"]["reported"]["value"][
        "interfaces"
    ].items():
...
def _pnp_interface_elements(cmd, interface, target_type, repo_endpoint, repo_id, login):
    interface_elements = []
    results = iot_pnp_interface_list(
        cmd, repo_endpoint, repo_id, interface, login=login
    )
    if results:
        interface_def = iot_pnp_interface_show(
            cmd, interface, repo_endpoint, repo_id, login
        )
        interface_contents = interface_def.get("contents")
        for content in interface_contents:
            if isinstance(content.get("@type"), list) and target_type in content.get(
                "@type"
            ):
...
def _device_interface_elements(
    ...
    try:
        ...
        interface_def = service_sdk.invoke_interface_command(
            device_id, INTERFACE_MODELDEFINITION, INTERFACE_COMMANDNAME, target_payload
        )
        if interface_def and interface_def.get("contents"):
            interface_contents = interface_def.get("contents")
            for content in interface_contents:
                if isinstance(
                    content.get("@type"), list
                ) and target_type in content.get("@type"):
                    interface_elements.append(content)
                elif content.get("@type") == target_type:
                    interface_elements.append(content)
        return interface_elements
