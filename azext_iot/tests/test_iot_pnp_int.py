...
if not all([_endpoint, _repo_id, _repo_cs]):
    raise ValueError(
        "Set azext_pnp_endpoint, azext_pnp_repository and azext_pnp_cs to run PnP model integration tests."
    )
...
class TestPnPModel(LiveScenarioTest):
        ...
        self.kwargs.update(
            {
                ...
                "interface_updated": "test_interface_updated_definition.json",
                "model": "test_model_definition.json",
                "model_updated": "test_model_updated_definition.json",
            }
        )
    ...
    def tearDown(self):
        change_dir()
        if exists(self.kwargs["interface_updated"]):
            os.remove(self.kwargs["interface_updated"])
        if exists(self.kwargs["model_updated"]):
            os.remove(self.kwargs["model_updated"])
    ...
    def test_interface_life_cycle(self):
        ...
        self.cmd(
            "iot pnp interface create -e {endpoint} --def {interface}",
            expect_failure=True,
        )

        # Error: Invalid Interface definition file
        self.cmd(
            "iot pnp interface create -e {endpoint} -r {repo} --def interface",
            expect_failure=True,
        )

        # Error: wrong path of Interface definition
        self.cmd(
            "iot pnp interface create -e {endpoint} -r {repo} --def interface.json",
            expect_failure=True,
        )
        ...
        self.cmd(
            "iot pnp interface list -e {endpoint} -r {repo}",
            checks=[
                self.greater_than("length([*])", 0),
                self.exists("[?urnId==`{}`]".format(self.kwargs["interface_id"])),
            ],
        )

        # Get Interface
        interface = self.cmd(
            "iot pnp interface show -e {endpoint} -r {repo} -i {interface_id}"
        ).get_output_in_json()
        ...
        interface_newContent = interface.replace(
            display_name, "{}-Updated".format(display_name)
        )
        interface_newContent = interface_newContent.replace("\n", "")
        fo = open(self.kwargs["interface_updated"], "w+", encoding="utf-8")
        fo.write(interface_newContent)
        fo.close()
        self.cmd(
            "iot pnp interface update -e {endpoint} -r {repo} --def {interface_updated}",
            checks=self.is_empty(),
        )

        # Publish Interface
        # Error must be published from partner tenant
        self.cmd(
            "iot pnp interface publish -e {endpoint} -r {repo} -i {interface_id}",
            expect_failure=True,
        )

        # Success: Delete Interface
        self.cmd(
            "iot pnp interface delete -e {endpoint} -r {repo} -i {interface_id}",
            checks=self.is_empty(),
        )
    ...
    def test_model_life_cycle(self):
        ...
        # Error: missing repo-id or login
        self.cmd(
            "iot pnp capability-model create -e {endpoint} --def {model}",
            expect_failure=True,
        )

        # Error: Invalid Capability-Model definition file
        self.cmd(
            "iot pnp capability-model create -e {endpoint} -r {repo} --def model",
            expect_failure=True,
        )

        # Error: wrong path of Capability-Model definition
        self.cmd(
            "iot pnp capability-model create -e {endpoint} -r {repo} --def model.json",
            expect_failure=True,
        )
        ...
        self.cmd(
            "iot pnp capability-model list -e {endpoint} -r {repo}",
            checks=[
                self.greater_than("length([*])", 0),
                self.exists("[?urnId==`{}`]".format(self.kwargs["model_id"])),
            ],
        )

        # Get Capability-Model
        model = self.cmd(
            "iot pnp capability-model show -e {endpoint} -r {repo} -m {model_id}"
        ).get_output_in_json()
        ...
        model_newContent = model_newContent.replace("\n", "")
        fo = open(self.kwargs["model_updated"], "w+", encoding="utf-8")
        fo.write(model_newContent)
        fo.close()
        self.cmd(
            "iot pnp capability-model update -e {endpoint} -r {repo} --def {model_updated}",
            checks=self.is_empty(),
        )

        # Publish Capability-Model
        # Error must be published from partner tenant
        self.cmd(
            "iot pnp capability-model publish -e {endpoint} -r {repo} -m {model_id}",
            expect_failure=True,
        )

        # Success: Delete Capability-Model
        self.cmd(
            "iot pnp capability-model delete -e {endpoint} -r {repo} -m {model_id}",
            checks=self.is_empty(),
        )
