...
APP_ID = os.environ.get("azext_iot_central_app_id_ppe")
...
device_template_path = get_context_path(
    __file__, "json/device_template_int_test_v1.json"
)
...
# TODO: when V1 API goes GA, remove --central-dns-suffix from commands here
...
class TestIotCentral(CaptureOutputLiveScenarioTest):
    ...
    def test_central_device_twin_show_fail(self):
        ...
        self.cmd(
            "iot central device twin show --app-id incorrect-app --device-id {} --central-dns-suffix azureiotcentral-ppe.com".format(
                device_id
            ),
            expect_failure=True,
        )
        ...
        self.cmd(
            "iot central device twin show --app-id {} --device-id incorrect-device --central-dns-suffix azureiotcentral-ppe.com".format(
                APP_ID
            ),
            expect_failure=True,
        )
    ...
    def test_central_device_twin_show_success(self):
        ...
        self.cmd(
            "iot central device twin show --app-id {} --device-id {} --central-dns-suffix azureiotcentral-ppe.com".format(
                APP_ID, device_id
            ),
    ...
    def test_central_monitor_events(self):
        ...
        self.cmd(
            "iot central diagnostics monitor-events --app-id {} -y --version v1 --central-dns-suffix azureiotcentral-ppe.com".format(
                APP_ID + "zzz"
            ),
            expect_failure=True,
        )
    ...
    def test_device_connect(self):
        ...
        self.cmd(
            "iot central device show --app-id {} -d {} --version v1 --central-dns-suffix azureiotcentral-ppe.com".format(APP_ID, device_id),
    ...
    def test_central_validate_messages_issues_detected(self):
        ...
        output = self._get_validate_messages_output(
            device_id, enqueued_time, max_messages=7
        )
    ...
    def test_central_user_methods_CRD(self):
        ...
        self.cmd(
            "iot central user show --app-id {} --user-id {} --version v1 --central-dns-suffix azureiotcentral-ppe.com".format(
                APP_ID, users[0].get("id")
            ),
        )

        result = self.cmd(
            "iot central user list --app-id {} --version v1 --central-dns-suffix azureiotcentral-ppe.com".format(APP_ID,),
        ).get_output_in_json()
    ...
    def test_central_api_token_methods_CRD(self):
        ...
        self.cmd(
            "iot central api-token show --app-id {} --token-id {} --version v1 --central-dns-suffix azureiotcentral-ppe.com".format(
                APP_ID, tokens[0].get("id")
            ),
        )

        result = self.cmd(
            "iot central api-token list --app-id {} --version v1 --central-dns-suffix azureiotcentral-ppe.com".format(APP_ID,),
        ).get_output_in_json()
    ...
    def test_central_device_template_methods_CRD(self):
        ...
        self.cmd(
            "iot central device-template show --app-id {} --device-template-id {} --version v1 --central-dns-suffix azureiotcentral-ppe.com".format(
                APP_ID, template_id
            ),
            checks=[
    ...
    def test_central_device_registration_info_registered(self):
        ...
        result = self.cmd(
            "iot central device registration-info --app-id {} -d {} --version v1 --central-dns-suffix azureiotcentral-ppe.com".format(
                APP_ID, device_id
            )
        )
        ...
        print(device_registration_info)
    ...
    def test_central_run_command(self):
        interface_id = "dtmi:digitalDistributionCenter2oumz6ipo8a:jacktest25;1"
        ...
        run_command_result = self.cmd(
            "iot central device command run --version v1 --central-dns-suffix azureiotcentral-ppe.com"
            " -n {}"
            " -d {}"
            " -i {}"
            " --cn {}"
        ...
        show_command_result = self.cmd(
            "iot central device command history --version v1 --central-dns-suffix azureiotcentral-ppe.com"
            " -n {}"
            " -d {}"
            " -i {}"
            " --cn {}"
        ...
        assert run_result["response"] == show_result[0]["response"]
    ...
    def test_central_device_registration_summary(self):
        ...
        result = self.cmd(
            "iot central diagnostics registration-summary --app-id {} --version v1 --central-dns-suffix azureiotcentral-ppe.com".format(APP_ID)
        )
    ...
    def _create_device(self, **kwargs) -> (str, str):
        ...
        command = "iot central device create --app-id {} -d {} --device-name {} --version v1 --central-dns-suffix azureiotcentral-ppe.com".format(
        ...
        if instance_of:
            ...
            checks.append(self.check("template", instance_of))
    ...
    def _create_users(self,):
        ...
        for role in Role:
            ...
            command = "iot central user create --app-id {} --user-id {} -r {} --email {} --version v1 --central-dns-suffix azureiotcentral-ppe.com".format(
    ...
    def _delete_user(self, user_id) -> None:
        self.cmd(
            "iot central user delete --app-id {} --user-id {} --version v1 --central-dns-suffix azureiotcentral-ppe.com".format(APP_ID, user_id),
        )
    ...
    def _create_api_tokens(self,):
        ...
        for role in Role:
            ...
            command = "iot central api-token create --app-id {} --token-id {} -r {} --version v1 --central-dns-suffix azureiotcentral-ppe.com".format(
    ...
    def _delete_api_token(self, token_id) -> None:
        self.cmd(
            "iot central api-token delete --app-id {} --token-id {} --version v1 --central-dns-suffix azureiotcentral-ppe.com".format(
                APP_ID, token_id
            )
        )

    def _wait_for_provisioned(self, device_id):
        command = "iot central device show --app-id {} -d {} --version v1 --central-dns-suffix azureiotcentral-ppe.com".format(APP_ID, device_id)
        while True:
    ...
    def _delete_device(self, device_id) -> None:
        self.cmd(
            "iot central device delete --app-id {} -d {} --version v1 --central-dns-suffix azureiotcentral-ppe.com".format(APP_ID, device_id),
        )
    ...
    def _create_device_template(self):
        ...
        self.cmd(
            "iot central device-template create --app-id {} --device-template-id {} -k '{}' --version v1 --central-dns-suffix azureiotcentral-ppe.com".format(
    ...
    def _delete_device_template(self, template_id):
        attempts = range(0, 10)
        command = "iot central device-template delete --app-id {} --device-template-id {} --version v1 --central-dns-suffix azureiotcentral-ppe.com".format(
            APP_ID, template_id
        )
    ...
    def _get_credentials(self, device_id):
        return self.cmd(
            "iot central device show-credentials --app-id {} -d {} --version v1 --central-dns-suffix azureiotcentral-ppe.com".format(
                APP_ID, device_id
            )
        ).get_output_in_json()
    ...
    def _get_validate_messages_output(
        ...
        output = self.command_execute_assert(
            "iot central diagnostics validate-messages --app-id {} -d {} --et {} --duration {} --mm {} -y --style json --version v1 --central-dns-suffix azureiotcentral-ppe.com".format(
    ...
    def _get_monitor_events_output(self, device_id, enqueued_time, asserts=None):
        ...
        output = self.command_execute_assert(
            "iot central diagnostics monitor-events -n {} -d {} --et {} --to 1 -y --central-dns-suffix azureiotcentral-ppe.com".format(
