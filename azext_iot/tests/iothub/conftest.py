...
def _iot_hub_provisioner(request, provisioned_user_identity):
    ...
    if provisioned_user_identity:
        user_identity_id = provisioned_user_identity
    base_create_command = f"iot hub create -n {name} -g {RG} --sku S1"

    hub_marker = request.node.get_closest_marker("hub_infrastructure")
    desired_location = None
    desired_tags = None
    desired_sys_identity = None
    desired_user_identity = None
    ...
    if desired_user_identity:
        user_identity_id = provisioned_user_identity_session["id"]
...
@pytest.fixture(scope="session")
def provisioned_event_hub_with_identity_session(provisioned_iot_hub_session, provisioned_event_hub_session) -> Optional[list]:
    role = "Azure Event Hubs Data Sender"
    scope = provisioned_event_hub_session[1]["id"]
    hub_principal_id = provisioned_iot_hub_session["identity"]["principalId"]
    user_id = provisioned_iot_hub_session["identity"]["UserAssignedIdentities"].keys()[0]
...
def provisioned_event_hub_session(request) -> Optional[list]:
    ...
    yield result
    if result:
        _event_hub_removal(result[0]["name"])
...
def _event_hub_provisioner(resource_name):
    """
    ...
    return namespace_obj, eventhub_obj, policy_obj
...
def _service_bus_provisioner(resource_name):
    """
    Create an event hub namespace, instance, and policy (if needed) and return the connection string for the policy.
    """
    namespace_name = generate_hub_depenency_id()
    ...
    return namespace_name, queue_name, topic_name, policy_name
