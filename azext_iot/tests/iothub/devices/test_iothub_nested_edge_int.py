...
class TestIoTHubNestedEdge(IoTLiveScenarioTest):
    def __init__(self, test_case):
        super(TestIoTHubNestedEdge, self).__init__(test_case, LIVE_HUB, LIVE_RG)
    ...
    def test_iothub_nested_edge(self):
        for auth_phase in DATAPLANE_AUTH_TYPES:
            ...
            for edge_device_id in edge_device_ids:
                self.cmd(
                    self.set_cmd_auth_type(
                        f"iot hub device-identity create -d {edge_device_id} -n {LIVE_HUB} -g {LIVE_RG} --ee",
            ...
            self.cmd(
                self.set_cmd_auth_type(
                    f"iot hub device-identity parent show -d {edge_device_ids[0]} -n {LIVE_HUB} -g {LIVE_RG}",
                    auth_type=auth_phase,
                ),
                expect_failure=True,
            )
            ...
            self.cmd(
                self.set_cmd_auth_type(
                    f"iot hub device-identity parent show -d {device_ids[0]} -n {LIVE_HUB} -g {LIVE_RG}",
                    auth_type=auth_phase,
                ),
                expect_failure=True,
            )
            ...
            self.cmd(
                self.set_cmd_auth_type(
                    f"iot hub device-identity parent set -d {device_ids[0]} --pd {device_ids[1]} -n {LIVE_HUB} -g {LIVE_RG}",
                    auth_type=auth_phase,
                ),
                expect_failure=True,
            )
            ...
            self.cmd(
                self.set_cmd_auth_type(
                    f"iot hub device-identity parent set -d {device_ids[2]} --pd {edge_device_ids[1]} -n {LIVE_HUB} -g {LIVE_RG}",
                    auth_type=auth_phase,
                ),
                expect_failure=True,
            )
            ...
            output = self.cmd(
                self.set_cmd_auth_type(
                    f"iot hub device-identity children list -d {edge_device_ids[0]} -n {LIVE_HUB} -g {LIVE_RG}",
                    auth_type=auth_phase,
                )
            )
            ...
            self.cmd(
                self.set_cmd_auth_type(
                    f"iot hub device-identity children remove -d {device_ids[0]} -n {LIVE_HUB} -g {LIVE_RG} --remove-all",
                    auth_type=auth_phase,
                ),
                expect_failure=True,
            )
            ...
            self.cmd(
                self.set_cmd_auth_type(
                    f"iot hub device-identity children remove -d {edge_device_ids[1]} -n {LIVE_HUB} -g {LIVE_RG} --remove-all",
                    auth_type=auth_phase,
                ),
                checks=self.is_empty(),
            )
            ...
            self.cmd(
                self.set_cmd_auth_type(
                    f"iot hub device-identity children remove -d {edge_device_ids[1]} -n {LIVE_HUB} -g {LIVE_RG}",
                    auth_type=auth_phase,
                ),
                expect_failure=True,
            )
            ...
            output = self.cmd(
                self.set_cmd_auth_type(
                    f"iot hub device-identity children list -d {edge_device_ids[1]} -n {LIVE_HUB} -g {LIVE_RG}",
                    auth_type=auth_phase,
                )
            )
