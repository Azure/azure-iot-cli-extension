...
class TestIotCentral(LiveScenarioTest):
    ...
    def test_central_device_show(self):
        ...
        self.cmd(
            'az iotcentral device-twin show --app-id incorrect-app --device-id "{}"'.format(
                DEVICE_ID
            ),
            expect_failure=True,
        )
        ...
        self.cmd(
            'az iotcentral device-twin show --app-id "{}" --device-id incorrect-device'.format(
                APP_ID
            ),
            expect_failure=True,
        )
    ...
    def test_central_validate_messages(self):
        # Test with invalid app-id
        self.cmd(
            "iotcentral app validate-messages --app-id {}".format(APP_ID + "zzz"),
            expect_failure=True,
        )
        # Ensure no failure
        # We cannot verify that the result is correct, as the Azure CLI for IoT Central does not support adding devices
