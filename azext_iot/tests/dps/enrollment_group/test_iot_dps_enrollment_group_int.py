...
def test_dps_enrollment_group_device_type_reference_round_trip(provisioned_csr_issuance):
    ...
    try:
        create_result = invoke_checked(
            cli,
            f"iot dps enrollment-group create {enrollment_scope} "
            f"--device-type-ref {quote(first_reference)}",
            description="Create semantic-reference DPS enrollment group",
        )
        enrollment_created = True
        created = create_result.as_json()
        assert created["deviceTypeRefs"] == [first_reference]

        shown = cli.invoke(f"iot dps enrollment-group show {enrollment_scope}").as_json()
        assert shown["deviceTypeRefs"] == [first_reference]

        listed = cli.invoke(
            f"iot dps enrollment-group list {resource_scope}"
        ).as_json()
        listed_enrollment = next(
            (
                enrollment
                for enrollment in listed
                if enrollment["enrollmentGroupId"] == enrollment_id
            ),
            None,
        )
        assert listed_enrollment
        assert listed_enrollment["deviceTypeRefs"] == [first_reference]

        preserved = cli.invoke(
            f"iot dps enrollment-group update {enrollment_scope} --provisioning-status disabled"
        ).as_json()
        assert preserved["deviceTypeRefs"] == [first_reference]

        replaced = cli.invoke(
            f"iot dps enrollment-group update {enrollment_scope} "
            f"--device-type-ref {quote(second_reference)}"
        ).as_json()
        assert replaced["deviceTypeRefs"] == [second_reference]

        cli.invoke(
            f"iot dps enrollment-group update {enrollment_scope} --remove-device-type-ref"
        )
        ...
        if enrollment_created:
            invoke_checked(
                cli,
                f"iot dps enrollment-group delete {enrollment_scope}",
                description="Delete semantic-reference DPS enrollment group",
            )
...
def test_dps_enrollment_group_x509_lifecycle(provisioned_iot_dps_module, auth_phases):
    ...
    for auth_phase in auth_phases:
        ...
        enrollment_list = cli.invoke(
            set_cmd_auth_type(
                f"iot dps enrollment-group list -g {dps_rg} --dps-name {dps_host_name}",
                auth_type=auth_phase,
                cstring=dps_cstring
            )
        ).as_json()
        ...
        enrollment_show = cli.invoke(
            set_cmd_auth_type(
                f"iot dps enrollment-group show -g {dps_rg} --dps-name {dps_host_name} --enrollment-id {enrollment_id}",
                auth_type=auth_phase,
                cstring=dps_cstring
            )
        ).as_json()
        ...
        with pytest.raises(BadRequestError):
            cli.invoke(
                set_cmd_auth_type(
                    f"iot dps enrollment-group compute-device-key -g {dps_rg} --dps-name {dps_host_name} "
        ...
        enrollment_list = cli.invoke(
            set_cmd_auth_type(
                f"iot dps enrollment-group registration list -g {dps_rg} --dps-name {dps_host_name} --enrollment-id "
                f"{enrollment_id}",
                auth_type=auth_phase,
                cstring=dps_cstring
            ),
        ...
        cli.invoke(
            set_cmd_auth_type(
                f"iot dps enrollment-group delete -g {dps_rg} --dps-name {dps_host_name} --enrollment-id {enrollment_id}",
                auth_type=auth_phase,
                cstring=dps_cstring
            ),
        )
...
def test_dps_enrollment_group_symmetrickey_lifecycle(provisioned_iot_dps_module, auth_phases):
    ...
    for auth_phase in auth_phases:
        ...
        enrollment2 = cli.invoke(
            set_cmd_auth_type(
                f"iot dps enrollment-group create -g {dps_rg} --dps-name {dps_host_name} --enrollment-id {enrollment_id2}",
                auth_type=auth_phase,
                cstring=dps_cstring
            ),
        ).as_json()
        ...
        enrollment_show = cli.invoke(
            set_cmd_auth_type(
                f"iot dps enrollment-group show -g {dps_rg} --dps-name {dps_host_name} --enrollment-id {enrollment_id2}",
                auth_type=auth_phase,
                cstring=dps_cstring
            ),
        ).as_json()
        ...
        enrollment_update = cli.invoke(
            set_cmd_auth_type(
                f"iot dps enrollment-group update -g {dps_rg} --dps-name {dps_host_name} --enrollment-id {enrollment_id2}"
        ...
        cli.invoke(
            set_cmd_auth_type(
                f"iot dps enrollment-group delete -g {dps_rg} --dps-name {dps_host_name} --enrollment-id {enrollment_id2}",
                auth_type=auth_phase,
                cstring=dps_cstring
            ),
        )
...
def test_dps_enrollment_twin_array(provisioned_iot_dps_module, auth_phases):
    ...
    for auth_phase in auth_phases:
        ...
        enrollment = cli.invoke(
            set_cmd_auth_type(
                f"iot dps enrollment-group create -g {dps_rg} --dps-name {dps_host_name} --enrollment-id {enrollment_group_id}"
                f" --cp {cert_path} --scp {cert_path} --provisioning-status {EntityStatusType.enabled.value} "
                f"--allocation-policy {AllocationType.geolatency.value} --iot-hubs {hub_hostname} --edge-enabled "
        ...
        cli.invoke(
            set_cmd_auth_type(
                f"iot dps enrollment-group delete -g {dps_rg} --dps-name {dps_host_name} --enrollment-id {enrollment_group_id}",
                auth_type=auth_phase,
                cstring=dps_cstring
            ),
        )
