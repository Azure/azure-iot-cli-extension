...
def writable_properties(resource):
    return {
        key: value
...
def test_get_put_update_preserves_supported_properties(
    ...
    assert body["properties"] == writable_properties(dps_resource)
    assert body["properties"]["disableLocalAuth"] is True
    assert "provisioningState" not in body["properties"]
    assert "serviceOperationsHostName" not in body["properties"]
    assert "idScope" not in body["properties"]
    assert "deviceRegistryNamespace" not in body["properties"]
    ...
    if not identity_args:
        ...
        # Partial identity updates preserve identity types not explicitly
        # removed, including identities selected by active ADR links.
...
def test_resource_lists_use_retained_paging(stable_client, mocked_response, dps_resource, resource_group):
    url = (
...
def test_delete_uses_retained_lro(stable_client, mocked_response):
