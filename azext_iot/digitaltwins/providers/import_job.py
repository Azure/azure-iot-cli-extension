...
class ImportJobProvider(DigitalTwinsProvider):
    ...
    def create(
        ...
        input_storage_account_cstring_op = self.cli.invoke(
            "az storage account show-connection-string -n {}".format(input_storage_account)
        )
        if not input_storage_account_cstring_op.success():
            raise CLIInternalError(
                "Unable to retrieve connection string for input storage account: {}".format(input_storage_account)
            )
        input_storage_account_cstring = input_storage_account_cstring_op.as_json()
        input_blob_url_op = self.cli.invoke(
            "storage blob url --connection-string {} --container-name {} --name {}".format(
                input_storage_account_cstring, input_blob_container, input_blob_name
            )
        )
        if not input_blob_url_op.success():
            raise CLIInternalError(
                "Unable to retrieve blob url for import data file: {} in {} container under {} account".format(
                    input_blob_name, input_blob_container, input_blob_container
                )
            )
        input_blob_url = input_blob_url_op.as_json()

        output_blob_url_op = self.cli.invoke(
            "storage blob url --account-name {} --container-name {} --name {}".format(
                output_storage_account, output_blob_container, output_blob_name
            )
        )
        if not output_blob_url_op.success():
            raise CLIInternalError(
                "Unable to retrieve blob url for import data file: {} in {} container under {} account".format(
                    output_blob_name, output_blob_container, output_blob_container
                )
            )
        output_blob_url = output_blob_url_op.as_json()
