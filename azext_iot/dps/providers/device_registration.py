...
def _read_csr_argument(value: str) -> str:
    ...
    if _CSR_PEM.fullmatch(value):
        return value
    try:
        x509.load_der_x509_csr(base64.b64decode(value, validate=True))
        return value
...
class DeviceRegistrationProvider:
    ...
    def create(
        ...
        if timeout is not None:
            from azext_iot.dps.services._registration import registration_deadline
        ...
        if csr is not None:
            body["csr"] = normalize_csr(_read_csr_argument(csr), self.registration_id)
        ...
        if timeout is not None:
            from azext_iot.dps.services._registration import register_with_deadline

            return register_with_deadline(self, body, timeout)
        return self._perform_registration(body)

    @staticmethod
    def _request_options(deadline):
        if deadline is None:
            return {}
        from azext_iot.dps.services._registration import _remaining
    ...
    def _invoke_operation(operation, deadline, **kwargs):
        ...
        from azext_iot.dps.services._registration import call_with_deadline

        return call_with_deadline(operation, deadline, **kwargs)
    ...
    def _wait_for_registration(self, response: _DeviceResponse, deadline=None):
        ...
        while current.status_code == 202:
            ...
            if request_deadline is not None:
                from azext_iot.dps.services._registration import _wait
