...
#### September ADR and DPS Integration Tests

This branch uses ADR management API `2026-04-01` and DPS management API
`2026-08-31`. The ADR suite covers only the compatible namespace, migration,
simple messaging, and system-assigned identity surface. It does not exercise the
removed preview credential/device/linking or ADR Software Updates commands.

Run from this worktree with a logged-in Azure CLI and a disposable resource
group. The test environments install this worktree's extension into their own
extension directories; they do not reuse the original branch's tox environment.
Set both resource-location variables below for ADR and DPS creation in
`centraluseuap`. Existing resources are not relocated. Ancillary Hub resources
retain their own placement requirements.

```bash
export azext_iot_testrg="<disposable-resource-group>"
export azext_iot_adr_resource_group="$azext_iot_testrg"
unset azext_iot_testdps

PRETTY_LOG=1 \
azext_iot_adr_location=centraluseuap \
azext_iot_dps_test_location=centraluseuap \
tox -e ADR-int,DPS-int -- -n 0 --reruns 0 -s --color=yes -ra
```

Both suites run sequentially without retries in this command. `-ra` includes skip reasons for
tests whose optional infrastructure is not configured. These are live tests:
use disposable resources and allow their cleanup to finish.

Set `azext_iot_testdps_hub` to an existing disposable GWv2 Hub in the test resource
group to enable the DPS linked-hub cases; otherwise those cases may skip. Leaving
`azext_iot_testdps` unset lets the fixtures create separate linked and unlinked DPS
instances instead of modifying one pinned instance.

#### ADR Integration Log Format

All ADR integration scenarios inherit `ADRLiveScenarioTest`. Each `self.cmd()`
invocation is logged once before execution, including setup, explicit polling,
expected failures, and cleanup. The displayed command resolves test placeholders
and redacts known credential arguments, signed URLs, and opaque inline payloads;
the command executed by the test SDK is unchanged. Do not add duplicate manual
command logs. Internal SDK HTTP polling is not shown as extra CLI commands.

Set `PRETTY_LOG=1` with `-s` (or `--capture=tee-sys`) to show the formatter live.
Headings are gold, commands blue, results warm gray, successes green, and
warnings/failures terracotta. Step timestamps use UTC and elapsed-duration lines
use the Greek delta symbol, including when a step raises an exception.

Leave `PRETTY_LOG` unset or set it to `0` for standard pytest warning-level log
capture without custom ANSI colors. Add `-o log_cli=true -o log_cli_level=WARNING`
to display those logs live.

#### Stable SDK Generation Provenance

The September DPS and ADR SDKs were generated from `azure-rest-api-specs`
commit `73741ee4ac19f2e12fd56abb44932e67f97ff2c5`, using each service's
`client.tsp` entrypoint. The toolchain was Node `24.14.1`, TypeSpec compiler
`1.15.0`, Azure TypeSpec libraries `0.71.0`, and
`@azure-tools/typespec-python` `0.63.5`.

Generation selected the API version explicitly and used `models-mode=none`,
`generate-typeddict=false`, and `no-async=true`, with samples and tests disabled.
Both management clients now accept `base_url`; the ADR client export is
`DeviceRegistryMgmtClient`, while DPS remains `IotDpsClient`. Their internal
generated package version is `1.0.0b1`, distinct from their service API versions.
