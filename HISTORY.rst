...
* Restored the 17 preview ``registry-device`` commands for CRUD, wait, authentication, attributes, and capabilities against the current modeless SDK. External-ID lookup follows every page; updates patch only supplied fields. Auth metadata redacts symmetric keys and only ``auth show-keys`` retrieves secrets. Composite link deletion remains retired.
