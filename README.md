...
- 🆕 **`0.33.0b9` Preview** expands `az iot adr ns` to 104 commands using the `2026-11-02-preview` Device Registry and Device Update APIs. It adds Update Instance lifecycle and Software Update content commands, Registry Device CRUD and child resources, namespace identity and migration, groups, jobs and runs, reports, certificate authorities and policies, and Hub/DPS/Software Updates links. It also refreshes namespace report and migration polling to follow the service's Azure-AsyncOperation contract. This is a cloud-only surface; resources that require an Azure IoT Operations custom location remain under `az iot ops ns`. See [HISTORY.rst](HISTORY.rst) for details. Install with `az extension add --name azure-iot --allow-preview`.
